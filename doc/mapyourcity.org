- login with open street map account => save password not in the database when it is possible to save it on the phone
- agreggate stats every week for every user and overwrite the history
- jedeR sollte den OSM account nutzen, wegen History speicherung
...
abfrage bei spielabruch, ob es irgend ein problem gegeben hat und auto bug report
alle 20-30 punkte eine interaktion zw. user und spiel: zB wenn du heute noch 10 pois validierst bekommst du zusatzpunkte
language support
layer: special, osm default, orthophotos
regex für email bei registration
netidee einreichen
...
* DOCUMENTATION
Virutal Environment starten: 
https://openshift.redhat.com/community/blogs/how-to-install-and-configure-a-python-flask-dev-environment-deploy-to-openshift
. venv/bin/activate
python runserver.py
